...
from nanobot.agent.guardrails import CAERequestFilter
...
class AgentLoop:
    ...
    def __init__(
        ...
        cae_guardrail_config: CAEGuardrailConfig | None = None,
    ):
        ...
        self.cae_guardrail_config = cae_guardrail_config or defaults.cae_guardrail
        ...
        self.cae_filter = CAERequestFilter(
            provider,
            model=self.model,
            config=self.cae_guardrail_config,
        )
    ...
    async def _process_message(
        ...
        filtered = await self._apply_cae_request_filter(session, msg, workflow=workflow)
    ...
    async def _apply_cae_request_filter(
        ...
        *,
        ...
        if not self.cae_guardrail_config.enable:
            return None

        decision = await self.cae_filter.evaluate(
            text=msg.content,
            workflow=workflow,
            is_command=msg.content.strip().startswith("/"),
            is_system=msg.channel == "system",
        )
        if decision.action == "allow":
            logger.info(
                "[CAE_FILTER] allow via {} category={} reason={}",
                decision.classifier,
                decision.category,
                decision.reason,
            )
        ...
        content = self.cae_guardrail_config.deny_template
        logger.info(
            "[CAE_FILTER] deny via {} category={} reason={} terms={}",
            decision.classifier,
            decision.category,
            decision.reason,
